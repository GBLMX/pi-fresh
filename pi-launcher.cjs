 ...
 *   [1] 启动 pi
 *   [2] 更新（pi update）
 *   [3] 扩展管理（packages / skills / extension 的禁用与启用）
 *   [0] 退出
 */
...
function listExtensions() {
  ...
  for (const f of readDir(EXT_DIR)) {
    ...
    // 单文件 extension（*.ts / *.ts.disabled）或目录 extension（含 index.ts）
...
function toggleExtension(item) {
  ...
  // skill / extension：重命名 .disabled 后缀
  const newPath = item.enabled
    ? item.path + ".disabled"
    : item.path.slice(0, -".disabled".length);
...
// ---------- 菜单 ----------
async function extManager() {
  for (;;) {
    const items = listExtensions();
    console.log("\n======== 扩展管理 ========");
    const kinds = { package: "包", skill: "技能", extension: "扩展" };
    items.forEach((it, i) => {
      const mark = it.enabled ? "[启用]" : "[禁用]";
      console.log(` [${i + 1}] ${mark} ${kinds[it.kind]}  ${it.name}`);
    });
    console.log(" [0] 返回主菜单");
    const a = (await ask("选择编号: ")).trim();
    if (a === "0" || a === "") return;
    const idx = parseInt(a, 10) - 1;
    if (idx >= 0 && idx < items.length) {
      console.log("  ->", toggleExtension(items[idx]));
    } else {
      console.log("  无效编号");
    }
  }
...
async function mainMenu() {
  for (;;) {
    ...
    console.log(" [3] 扩展管理");
    console.log(" [0] 退出");
    const a = (await ask("选择: ")).trim();
    if (a === "1") return launchPi();
    if (a === "2") doUpdate();
    else if (a === "3") await extManager();
    else if (a === "0") {
      console.log("退出。");
      process.exit(0);
    } else {
